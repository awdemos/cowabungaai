...
export const mockGetKeys = (keys: APIKeyRow[]) => {
  server.use(
    http.get(`${process.env.LEAPFROGAI_API_BASE_URL}/cowabunga/v1/auth/api-keys`, () =>
...
export const mockCreateApiKey = (api_key = `lfai_${faker.string.uuid()}`) => {
  server.use(
    http.post(
      `${process.env.LEAPFROGAI_API_BASE_URL}/cowabunga/v1/auth/api-keys`,
      async ({ request }) => {
...
export const mockRevokeApiKey = () => {
  server.use(
    http.delete(
      `${process.env.LEAPFROGAI_API_BASE_URL}/cowabunga/v1/auth/api-keys/:id`,
