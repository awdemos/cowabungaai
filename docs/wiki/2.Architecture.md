...
erDiagram
    ...
    USER_ROLES {
        AI_APPLICATION_DEVELOPERS "Software engineers building AI-powered applications"
        SYSTEM_ADMINISTRATORS "DevOps engineers managing AI infrastructure"
        END_USERS "Users interacting with AI chat assistants"
    }
    
    USER_SCENARIOS {
        LLM_INFERANCE "Access to LLM inference APIs for chat completions"
        EMBEDDING_GENERATION "Text embedding generation for vector search"
        AUDIO_TRANSCRIPTION "Audio file transcription to text"
        KUBERNETES_DEPLOYMENT "Kubernetes deployment management"
        GPU_CONFIGURATION "GPU resource configuration and monitoring"
        NATURAL_LANGUAGE_CHAT "Natural language chat interface interaction"
        CONVERSATION_HISTORY "Conversation history management and retrieval"
        FILE_UPLOAD "File upload and storage for audio processing"
    }
```
